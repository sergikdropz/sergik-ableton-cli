...
class EnhancedClipEditor {
    ...
    handleParameterChange(param, value) {
        ...
        // Send to API (Electron app)
        if (window.sergikAPI) {
            window.sergikAPI.setClipProperty?.({ param, value }).catch(err => 
                console.warn('[EnhancedClipEditor] Failed to set clip property:', err)
            );
        }
    ...
    handleBPMChange(bpm) {
        ...
        // Send to API (Electron app)
        if (window.sergikAPI) {
            window.sergikAPI.setClipBPM?.(bpm).catch(err => 
                console.warn('[EnhancedClipEditor] Failed to set clip BPM:', err)
            );
        }
    }
    ...
    handleWarpMarkerAdded(detail) {
        ...
        // Send to API (Electron app)
        if (window.sergikAPI) {
            window.sergikAPI.addWarpMarker?.(detail).catch(err => 
                console.warn('[EnhancedClipEditor] Failed to add warp marker:', err)
            );
        }
    }
    ...
    handleNotesTransposed(detail) {
        ...
        // Send to API (Electron app)
        if (window.sergikAPI) {
            window.sergikAPI.transposeNotes?.(detail).catch(err => 
                console.warn('[EnhancedClipEditor] Failed to transpose notes:', err)
            );
        }
    }
...
if (typeof window !== 'undefined') {
    ...
    window.ClipEditorSidebar = ClipEditorSidebar;
    window.ClipEditorMain = ClipEditorMain;
}
